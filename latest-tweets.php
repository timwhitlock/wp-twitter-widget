...
function latest_tweets_render( $screen_name, $count, $rts, $ats, $pop = 0, $loklak ){
    try {
        if( $loklak == true ){
            ...
            $tweets = array();
            while( $batch = $loklak->search('', null, null, $screen_name, $count) ){
                $batch = json_decode($batch, true);
                $batch = json_decode($batch['body'], true); 
                // var_dump($batch['statuses']);
                //die();
                $max_id = null;
                foreach( $batch['statuses'] as $tweet ){
                    ...
                    if( $pop > ( $tweet['retweet_count'] + $tweet['favourites_count'] ) ){
